...
def offsetCount(soup) :
        productCount = soup.find('span', class_ = 'count-products' ).text
        count = re.findall('(\d+)', productCount)
        final_count = (int(count[0]) // 20) + 1
        return final_count

jsonfile = {}
jsonCount = 0

for k in range(offsetCount(soup)) :
        offsetUrl = url + page_number + str(k * 20)
        html_code = requests.get(offsetUrl, cookies = cookies, headers = headers)
        soup = BeautifulSoup(html_code.text, 'lxml')
        ...
        for div in products:
                ...
                link = 'https://www.dns-shop.ru' + div.find('a', class_ = 'ec-price-item-link').attrs['href']
                price = div.find('div', class_= 'price_g' )
                newPrice = price.find('span').text
                try:
                        oldPrice = div.find('div', class_='markdown-price-old').text
                except AttributeError :
                        oldPrice = 0

                #making JSON with products
                productInfo = {
                        'title': title,
                        'link' : link,
                        'newPrice' : newPrice,
                ...
                jsonfile[jsonCount] = productInfo
                jsonCount += 1

#saving JSON
filename = 'data_' + str(datetime.now()) + '.json'
